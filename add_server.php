...
// Per riempire i campi in caso di errore
$postServerName = $_POST['server_name'] ?? '';
$postSubdomain = $_POST['subdomain'] ?? '';
$postType = $_POST['type'] ?? '';
    ...
    if (!$serverName || !$subdomain || !$userId) {
        ...
        if (!$vm) {
            ...
            if (!$successInsert) {
                ...
                // Comando installazione server remoto (da adattare)
                $sshUser = 'diego';
...
<form method="POST" action="">
    <div class="mb-3" style="max-width: 400px;">
        ...
        <input type="text" name="server_name" id="server_name" class="form-control" required value="<?= htmlspecialchars($postServerName) ?>">
    </div>

    <div class="mb-3" style="max-width: 400px;">
        <label for="subdomain" class="form-label">Sottodominio (es. mc123)</label>
        <div class="input-group">
            <input type="text" name="subdomain" id="subdomain" class="form-control" required value="<?= htmlspecialchars($postSubdomain) ?>">
    ...
    <div class="mb-3" style="max-width: 400px;">
        ...
        <select name="type" id="type" class="form-select" required>
            ...
            <option value="spigot" <?= $postType === 'spigot' ? 'selected' : '' ?>>Spigot</option>
            <option value="modpack" <?= $postType === 'modpack' ? 'selected' : '' ?>>Modpack</option>
        </select>
    </div>

    <div class="mb-3" id="modpack_selector" style="max-width: 400px; display: <?= $postType === 'modpack' ? 'block' : 'none' ?>;">
