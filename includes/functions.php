...
function avviaTunnelNgrokTcp(int $portaLocale = 25565): ?array {
    ...
    $cmd = "sudo -u www-data nohup ngrok tcp $portaLocale --log=stdout --log-format=json > /tmp/ngrok.log 2>&1 & echo $!";
...
function startNgrokTcpTunnel(int $localPort = 25565): ?array {
    ...
    $cmd = "sudo -u www-data nohup ngrok tcp $localPort --log=stdout > /dev/null 2>&1 &";
